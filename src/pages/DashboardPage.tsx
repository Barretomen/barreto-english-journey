  ...
  return (
    <div className="page dashboard-page">
      ...
      <section className="journey-card" aria-labelledby="journey-overview-title">
        ...
        <div className="journey-card__action">{data.currentLesson ? <><span className="lesson-index">{data.currentLesson.levelCode} · MÓDULO {data.currentLesson.moduleNumber} · AULA {data.currentLesson.moduleLessonNumber}</span><h3>{data.currentLesson.title}</h3><p>{data.currentLesson.summary}</p><Link className="button button--signal" to={`/lesson/${data.currentLesson.id}`}>{firstStart ? 'Começar minha primeira aula' : 'Continuar aprendendo'} <ArrowRight aria-hidden="true" /></Link></> : <p>Aguarde o professor liberar a próxima aula.</p>}</div>
      </section>

      <div className="dashboard-grid">
        <section className="current-module"><div className="section-title"><div><span className="section-kicker">MÓDULO ATUAL</span><h2>{data.currentLesson?.moduleTitle ?? 'Primeiros passos'}</h2></div><BookOpen aria-hidden="true" /></div>{data.currentLesson ? <Link to={`/lesson/${data.currentLesson.id}`} className="module-lesson"><span>{data.currentLesson.levelCode} · M{data.currentLesson.moduleNumber} · AULA {data.currentLesson.moduleLessonNumber}</span><strong>{data.currentLesson.title}</strong><small>{data.currentLesson.state === 'completed' ? 'Concluída' : data.currentLesson.progressPercent >= 95 ? 'Pronta para concluir' : data.currentLesson.progressPercent > 0 ? 'Em andamento' : 'Disponível'}</small><ArrowRight aria-hidden="true" /></Link> : <p>Nenhuma aula liberada no momento.</p>}</section>
        <section className="route-preview"><div className="section-title"><div><span className="section-kicker">SUA ROTA</span><h2>A1 ao C2</h2></div><Link to="/journey">Ver jornada</Link></div><div className="route-line" aria-label="Trecho atual da jornada">{routeWindow.map((lesson) => <span key={lesson.id} className={`route-dot route-dot--${lesson.state}`} aria-label={`${lesson.title}: ${lesson.state}`}><b>{lesson.globalOrder}</b></span>)}</div><p>{data.completedLessons} de {data.totalPublishedLessons} aulas concluídas</p></section>
      </div>
    </div>
  )
}
