...
export default function AdminPage() {
  ...
  async function generate(lessonId: number) {
    setGeneratingLesson(lessonId)
    setAudioMessage('')
    try {
      const result = await generateLessonAudio(lessonId)
  ...
  return <div className="page admin-page">
    <header className="page-heading"><span className="field-label"><i />VISÃO DO PROFESSOR</span><h1>Alunos</h1><p>Acompanhe o essencial e libere o próximo passo com intenção.</p></header>
    ...
    <section className="admin-audio" aria-labelledby="admin-audio-title">
      ...
      <div className="audio-lesson-list">{audio.data?.filter((lesson) => lesson.totalCount > 0).map((lesson) => {
        const busy = generatingLesson === lesson.lessonId
        return <article className="audio-lesson-row" key={lesson.lessonId}>
          ...
          <Button variant="secondary" disabled={busy || (lesson.readyCount === lesson.totalCount && lesson.failedCount === 0)} onClick={() => void generate(lesson.lessonId)}>{busy ? <LoaderCircle className="is-spinning" aria-hidden="true" /> : <Volume2 aria-hidden="true" />}{busy ? 'Gerando…' : lesson.failedCount > 0 ? 'Tentar novamente' : 'Gerar ausentes'}</Button>
        </article>
      })}</div>
